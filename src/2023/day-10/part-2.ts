...
//
// This rule makes things way harder, otherwise we could just flood fill...
...
export default function (blob: string) {
  ...
  let path = new Set([...scoutPaths.flatMap((v) => Array.from(v))])

  // Cleanup the map
  //
  // - Replace all unvisited pipes with empty spaces
  // - Replace the start with the inferred type
  let cleanMap = new Map(map)
  for (let [p, type] of cleanMap) {
    if (!path.has(p)) cleanMap.set(p, Type.Empty)
    if (type === Type.Start) cleanMap.set(p, infertTypeOf(map, p))
  }

  let total = 0
  let inside = false
  for (let type of cleanMap.values()) {
    if (type & Type.Empty && inside) {
      total += 1
    } else if (type & (Type.Vertical | Type.BR | Type.BL)) {
      inside = !inside
    }
  }
  return total
...
function infertTypeOf(map: Map<Point, Type>, p: Point) {
  ...
  for (let [type, [up, right, down, left]] of lookup) {
    if (
      (up === null || up.includes(map.get(p.up()))) &&
      (right === null || right.includes(map.get(p.right()))) &&
      (down === null || down.includes(map.get(p.down()))) &&
      (left === null || left.includes(map.get(p.left())))
    ) {
      return type
    }
  }
