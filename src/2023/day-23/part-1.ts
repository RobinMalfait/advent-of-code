...
export default function (blob: string) {
  ...
  for (let point of intersections) {
    ...
    while (todo.length > 0) {
      ...
      if (tile.type === Tile.Slopes) {
        let nextPoint = match(tile.dir, {
          [Direction.North]: () => location.up(),
          [Direction.East]: () => location.right(),
          [Direction.South]: () => location.down(),
          [Direction.West]: () => location.left(),
        })
        if (!grid.has(nextPoint)) continue // Off-grid
        if (seen.has(nextPoint)) continue // Already seen
        todo.push([nextPoint, distance + 1])
        seen.add(nextPoint)
      }
    }
  }
