export default function (blob: string, teaspoons = 100) {
  ...
  for (let weights of divide(teaspoons, ingredients.length)) {
    ...
    let calories = ingredients.map((ingredient) => ingredient.properties.get('calories'))
    if (
      multiply([calories], transpose([weights]))
        .flat()
        .reduce((a, b) => a + b, 0) === 500
    ) {
      score = Math.max(score, total / 500 /* Don't include calories */)
    }
  }
